...
export function NexusGraph() {
  ...
  const repoNodes = useMemo(() => {
    const nodes: Array<{ slug: string; x: number; y: number; fed: FederationId }> = [];
  ...
  return (
    <div className="relative w-full h-[700px] bg-[#050505] rounded-2xl border border-[#D4AF37]/20 overflow-hidden shadow-2xl">
      ...
      {hovered ? (
        <div className="absolute top-6 right-6 p-6 bg-black/90 border border-[#D4AF37] text-[#D4AF37] backdrop-blur-md rounded-lg transition-opacity">
          <h3 className="text-xs tracking-[0.3em] uppercase">{FEDERATIONS[hovered].name}</h3>
